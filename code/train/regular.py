...
def train(train_data, val_data, model, args):
    ...
    for ep in range(args.train_epochs):
        ...
        if ep % 10 == 0:
            acc, std, _, _, _, _ = test(train_data, model, args, args.val_episodes, False,
        ...
        cur_acc, cur_std, cur_f1, cur_f1_std, cur_mcc, cur_mcc_std = test(val_data, model, args, args.val_episodes, False,
                                val_gen.get_epoch())
        ...
        if cur_acc > best_acc:
            ...
            # save current model
            print( colored( "{}, Save cur best model to {}".format(
                datetime.datetime.now().strftime('%02y/%02m/%02d %H:%M:%S'),
                best_path) , 'magenta' ))
...
def train_one(task, model, opt, args, grad):
    ...
    _, loss, _, _ = model['clf'](XS, YS, XQ, YQ)
...
def test(test_data, model, args, num_episodes, verbose=True, sampled_tasks=None, target='val'):
    ...
    for task in sampled_tasks:
        res_acc, res_f1, res_mcc = test_one(task, model, args)
        acc.append(res_acc)
        f1.append(res_f1)
        mcc.append(res_mcc)
...
def test_one(task, model, args):
    ...
    # Apply the classifier
    acc, _, f1, mcc = model['clf'](XS, YS, XQ, YQ)
