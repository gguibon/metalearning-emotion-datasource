...
class BASE(nn.Module):
    ...
    @staticmethod
    def compute_acc(pred, true, dim=1):
        '''
            Compute the accuracy.
            @param pred: batch_size * num_classes
            @param true: batch_size
        '''
        return torch.mean((torch.argmax(pred, dim=dim) == true).float()).item()

    @staticmethod
    def compute_f1(pred, true, dim=1):
        '''
            Compute the weighted f1 score.
            @param pred: batch_size * num_classes
            @param true: batch_size
        '''
        _, y_pred = torch.max(pred, dim)

        f1 = f1_score(true.cpu().detach().numpy(), y_pred.cpu().detach().numpy(), average='weighted')

        return f1

    @staticmethod
    def compute_mcc(pred, true, dim=1):
        '''
